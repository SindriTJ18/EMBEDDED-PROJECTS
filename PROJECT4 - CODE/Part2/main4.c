...
static unsigned int gpioOUT = 5;             // pin 11 (GPIO17) 
...
static bool         outOn = 0;                // used to invert state of LED

module_param(numberPresses, int, S_IRUGO);
MODULE_PARM_DESC(numberPresses, "The amount of presses");
...
static int __init erpi_gpio_init(void) 
{
    ...
    gpio_request(gpioOUT, "sysfs");          // request LED GPIO
    gpio_direction_output(gpioOUT, outOn);   // set in output mode and on 
    // gpio_set_value(gpioOUT, outOn);       // not reqd - see line above
...
{   
    ...
    gpio_set_value(gpioOUT, 0);              // turn the LED off
    gpio_unexport(gpioOUT);                  // unexport the LED GPIO   
    free_irq(irqNumber, NULL);               // free the IRQ number, no *dev_id   
    gpio_unexport(gpioButton);               // unexport the Button GPIO   
    gpio_free(gpioOUT);                      // free the LED GPIO
...
static irq_handler_t erpi_gpio_irq_handler(unsigned int irq, 
                                           void *dev_id, struct pt_regs *regs) 
{   
    outOn = !outOn;                          // toggle the LED state   
    gpio_set_value(gpioOUT, outOn);          // set LED accordingly  
